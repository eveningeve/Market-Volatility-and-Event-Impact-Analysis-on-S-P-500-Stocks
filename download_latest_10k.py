...
def combine_indices_to_csv(index_dir, cik_set=None):
    """
    Combine all index files into one CSV, keeping ONLY the latest 10-K per company per year.
    """
    rows = []

    # Temporary dictionary to dedupe:
    # (CIK, YEAR) -> record
    latest_filing = {}
    ...
    for index_path in sorted(glob(os.path.join(index_dir, "*.idx"))):
        ...
        # Skip header lines until dashed separator
        data_started = False
        for line in lines:
            if line.startswith("-----"):
                data_started = True
            ...
            # Only 10-K and 10-K/A
            if form_type not in ["10-K", "10-K/A"]:
                continue

            # Only track companies in your list
            ...
            # If first time seeing this year → save it
            if key not in latest_filing:
                latest_filing[key] = [form_type, company_name, cik, date_filed, filename, url]
            else:
                # Keep the one with the MOST RECENT date_filed
                if date_filed > latest_filing[key][3]:
                    latest_filing[key] = [form_type, company_name, cik, date_filed, filename, url]

    # Convert dict back to list
    rows = list(latest_filing.values())
    ...
    print(f"Total latest 10-K forms found: {len(rows)}")
